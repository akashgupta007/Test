...
function getRowVal(rowid) {
    ...
    if (tbl != null) {
        ...
        $("#CityId").val($tds.cells[1].innerText.trim());
        $("#OrderNo").val($tds.cells[2].innerText.trim());
        $("#CityName").val($tds.cells[3].innerText.trim());
    }
}

function RowDelete(CityId) {
    try {
        ...
        $.post("LocationDetailDelete", form.serialize(),
           function (response) {
...
function LocationDetailGet() {
    try {
        $("#MsgDiv").removeAttr("class");
        $("#lblError").html("");
        $("#MsgDiv").hide();
        $.post("LocationDetailGet", function (response) {
    ...
    $("#btnInsert").click(
        function (e) {
            try {
                ...
                $.post("LocationDetailInsert", form.serialize(),
                    function (response) {
                        if (response.Flag == 0) {  //Successfully
                            $("#data").html("");
                            $("#data").html(response.Html);
    ...
    $("#btnUpdate").click(
     function (e) {
         try {
             ...
             $.post("LocationDetailUpdate", form.serialize(),
